...
def create_sample_habits():
    ...
    return [
        Habit(habit_id=1, name='Workout', description='Hit the gym twice a week', periodicity='weekly',
              streak_in_weeks=45, streak_in_days=45 * 7, start_date=(subtract_date(weeks=45)),
              current_streak_date=datetime.now().timestamp()
              ),
        Habit(habit_id=2, name='Eat healthy', description='Skip soda and hot dogs', periodicity='daily',
              streak_in_weeks=18, streak_in_days=18 * 7, start_date=(subtract_date(weeks=18)),
              current_streak_date=datetime.now().timestamp()
              ),
        Habit(habit_id=3, name='Sleep well', description='Sleep at least 8 hours per night', periodicity='daily',
              streak_in_weeks=50, streak_in_days=50 * 7, start_date=(subtract_date(weeks=50)),
              current_streak_date=datetime.now().timestamp()
              ),
        Habit(habit_id=4, name='Family', description='''Be home early for dinner, read bedtime stories to Karl, 
              ...
              streak_in_weeks=323, streak_in_days=323 * 7, start_date=(subtract_date(weeks=323)),
...
class HabitTracker:
    ...
    def __init__(self):
        ...
        self.habits = Habit.from_db_to_list(self.database.select_all())
        self.dataframe = pandas.DataFrame(self.habits, columns=['Id', 'Name', 'Description', 'Periodicity',
                                                                ...
                                                                'Streak (weeks)'])
        self.dataframe.sort_values('Streak (days)', inplace=True)
